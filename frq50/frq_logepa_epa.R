...
top20
list <- c('logepa_ddr' , 
          'logepa_pfa', 'logepa_pfa', 'logepa_pfa', 
          'logepa_pfa', 'logepa_pfa')
...
reg <- lmer( logepa_pfa ~ logepa_ddr + s__Dorea_formicigenerans + logepa_ddr*s__Dorea_formicigenerans 
             #+agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logepa_pfa ~ logepa_ddr + s__Agathobaculum_butyriciproducens + logepa_ddr*s__Agathobaculum_butyriciproducens 
             #+agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logepa_pfa ~ logepa_ddr + s__Asaccharobacter_celatus + logepa_ddr*s__Asaccharobacter_celatus 
             #+agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logepa_pfa ~ logepa_ddr + s__Ruthenibacterium_lactatiformans + logepa_ddr*s__Ruthenibacterium_lactatiformans 
             #+agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logepa_pfa ~ logepa_ddr + s__Adlercreutzia_equolifaciens + logepa_ddr*s__Adlercreutzia_equolifaciens 
             #+agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
write.table(a, file = "./data/frq_logepa_epa.pcl",  sep = '\t', quote = F, eol = '\n',  row.names=T)
