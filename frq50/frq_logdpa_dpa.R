...
top20
list <- c('logdpa_ddr' , 
          'logepa_pfa', 'logdpa_pfa', 'logdpa_pfa', 
          'logdpa_pfa', 'logdpa_pfa')
...
reg <- lmer( logdpa_pfa ~ logdpa_ddr + s__Dorea_formicigenerans + logdpa_ddr*s__Dorea_formicigenerans 
             #+agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logdpa_pfa ~ logdpa_ddr + s__Agathobaculum_butyriciproducens + logdpa_ddr*s__Agathobaculum_butyriciproducens 
             #+agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logdpa_pfa ~ logdpa_ddr + s__Asaccharobacter_celatus + logdpa_ddr*s__Asaccharobacter_celatus 
             #+agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logdpa_pfa ~ logdpa_ddr + s__Ruthenibacterium_lactatiformans + logdpa_ddr*s__Ruthenibacterium_lactatiformans 
             #+agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logdpa_pfa ~ logdpa_ddr + s__Adlercreutzia_equolifaciens + logdpa_ddr*s__Adlercreutzia_equolifaciens 
             #+agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
write.table(a, file = "./data/frq_logdpa_dpa.pcl",  sep = '\t', quote = F, eol = '\n',  row.names=T)
