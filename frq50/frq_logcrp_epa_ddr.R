...
reg <- lmer( logcrp_plasma ~ logepa_ddr + s__Dorea_formicigenerans + logepa_ddr*s__Dorea_formicigenerans 
             +agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logcrp_plasma ~ logepa_ddr + s__Agathobaculum_butyriciproducens + logepa_ddr*s__Agathobaculum_butyriciproducens 
             +agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logcrp_plasma ~ logepa_ddr + s__Asaccharobacter_celatus + logepa_ddr*s__Asaccharobacter_celatus 
             +agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logcrp_plasma ~ logepa_ddr + s__Ruthenibacterium_lactatiformans + logepa_ddr*s__Ruthenibacterium_lactatiformans 
             +agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
reg <- lmer( logcrp_plasma ~ logepa_ddr + s__Adlercreutzia_equolifaciens + logepa_ddr*s__Adlercreutzia_equolifaciens 
             +agemlvs+abx_ddr+probx_ddr+bristol_ddr+ smoke12 +calor_fs_dr_ddr 
             +(1 | ID1), data=meta_bispecies)
a<- rbind(a, cbind(beta = reg@beta[length(reg@beta)],  anova(reg)[nrow(anova(reg)),]))
...
write.table(a, file = "./data/frq_int_logCRP_logepa_ddr.pcl",  sep = '\t', quote = F, eol = '\n',  row.names=T)
...
ggplot(a, aes(name, beta)) +
  ...
  coord_flip()+
  labs(title ="frq_int_logCRP_logepa_ddr.pcl")+
