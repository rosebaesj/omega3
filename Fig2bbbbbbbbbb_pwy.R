...
s1<-read_sas("./data/species_s1.sas7bdat")
s2<-read_sas("./data/species_s2.sas7bdat")
s3<-read_sas("./data/species_s3.sas7bdat")
s4<-read_sas("./data/species_s4.sas7bdat")

midnumber_aliasid<-read.csv(file="./data_generated/midnumber_aliasid.csv")
id_key<-read.csv(file="./data/idkey.csv") # SAS is id not ID1
...
transform_data<-function(data1, label1, label2){
  data1<-inner_join(data1,id_key, by="id")
  
  data1$ID1_stn<-paste0(data1$ID1, label1)
  
  data2<-as.data.frame(t(data1[,-(ncol(data1)-1):-(ncol(data1))]))
  colnames(data2)<-data1$ID1_stn
